...
export function RefCodeBadge({ variant = "inline", label }: { variant?: Variant; label?: string }) {
  const { t } = useLanguage();
  const finalLabel = label ?? t("রেফারেন্স কোড", "Reference code");
  const copyAria = t("কপি করুন", "Copy");
  ...
  if (variant === "sidebar") {
    return (
      <div className="mx-4 mb-3 px-3 py-2 rounded-lg flex items-center justify-between gap-2" style={{ background: "color-mix(in oklab, var(--brand-gold-500) 8%, transparent)", border: "1px solid color-mix(in oklab, var(--brand-gold-500) 22%, transparent)" }}>
        ...
        <button onClick={copy} aria-label={copyAria} className="p-1.5 rounded-md hover:bg-white/10 transition shrink-0" style={{ color: "var(--brand-gold-500)" }}>
  ...
  if (variant === "menu") {
    return (
      <div className="px-3 py-2 border-b flex items-center justify-between gap-2" style={{ borderColor: "var(--brand-line)" }}>
        <div className="min-w-0">
          <div className="text-[10px] uppercase tracking-wider text-slate-500">{finalLabel}</div>
          <div className="text-sm font-mono tracking-wider truncate" style={{ color: "var(--brand-ink)" }}>{refCode}</div>
        </div>
        <button onClick={copy} aria-label={copyAria} className="p-1.5 rounded-md hover:bg-slate-100 transition shrink-0">
  ...
  if (variant === "header") {
    return (
      <button onClick={copy} title={copyAria} className="hidden sm:flex items-center gap-1.5 px-2.5 py-2 bg-white rounded-lg border hover:shadow-sm transition" style={{ borderColor: "var(--brand-line)" }}>
  ...
  return (
    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md border bg-white" style={{ borderColor: "var(--brand-line)" }}>
      <span className="text-[10px] uppercase tracking-wider text-slate-500">{finalLabel}</span>
      <span className="text-sm font-mono tracking-wider" style={{ color: "var(--brand-ink)" }}>{refCode}</span>
      <button onClick={copy} aria-label={copyAria} className="p-1 rounded hover:bg-slate-100 transition">
        {copied ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-slate-500" />}
      </button>
    </div>
  );
