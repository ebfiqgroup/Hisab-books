...
type Txn = { type: "income" | "expense"; amount: number; occurred_on: string; category: string; note: string | null };
...
function CalendarPage() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("transactions").select("type,amount,occurred_on,category,note").gte("occurred_on", startISO).lt("occurred_on", endISO);
